    ...
    const verifyEmail = async () => {
      try {
        ...
        const response = await fetch("http://localhost:8080/api/auth/verify", {
          ...
          body: JSON.stringify({ token: "example-verification-token" }), // Replace with actual token
        });
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        setErrorMessage(errorData.error || "Failed to resend verification email.");
      }
    } catch (error) {
      setErrorMessage("Failed to connect to the server. Please try again.");
    } finally {
    ...
    <div className="min-h-screen flex items-center justify-center bg-black p-4">
      <div className="flex flex-col items-center gap-6 max-w-md text-center">
        ...
        <div className="relative">
          {isLoading ? (
            <Loader2 className="w-32 h-32 text-violet-400 animate-spin" strokeWidth={1.5} />
          ) : isVerified ? (
            <CheckCircle2 className="w-32 h-32 text-violet-400" strokeWidth={1.5} />
          ) : (
        ...
        <div className="space-y-3">
          {isLoading ? (
            ...
            <>
              ...
              <p className="text-red-400/80">{errorMessage || "Unable to verify your email. Please try again."}</p>
            </>
          )}
        </div>
