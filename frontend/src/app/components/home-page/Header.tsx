...
export default function Header(): React.ReactElement {
  ...
  const [hoveredDevMatchSection, setHoveredDevMatchSection] = useState<string | null>(null)
  const [hoveredAccountSection, setHoveredAccountSection] = useState<string | null>(null)
      ...
      <header className="fixed top-6 left-1/2 -translate-x-1/2 z-50 w-11/12 max-w-2xl">
        <div className="bg-violet-500/10  backdrop-blur-md border border-violet-500/20 rounded-2xl px-8 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-8">
            ...
            <div className="relative" onMouseEnter={handleDevMatchMouseEnter} onMouseLeave={handleDevMatchMouseLeave}>
              <button
                onClick={() => setIsDevMatchOpen(!isDevMatchOpen)}
                className="flex items-center gap-2 text-white/80 hover:text-white transition-colors text-sm font-medium"
              >
                What is DevMatch?
                <ChevronDown size={16} className={`transition-transform ${isDevMatchOpen ? "rotate-180" : ""}`} />
              </button>
                ...
                <div className="absolute top-full mt-0 left-1/2 -translate-x-1/2 w-80 pt-2">
                  <div className="bg-black/95 backdrop-blur-md border border-violet-500/20 rounded-xl p-6 space-y-6">
                    ...
                    <div
                      ...
                      onMouseEnter={() => setHoveredDevMatchSection("description")}
                      onMouseLeave={() => setHoveredDevMatchSection(null)}
                    >
                      <div
                        className={`absolute left-0 top-0 bottom-0 w-1 bg-violet-400 rounded-full transition-all duration-300 ${hoveredDevMatchSection === "description" ? "scale-y-100 opacity-100" : "scale-y-0 opacity-0"}`}
                      />
                      <h3 className="text-white font-semibold text-sm">Description</h3>
                      <p className="text-white/80 text-xs leading-relaxed">
                        DevMatch connects developers with opportunities that match their skills, experience, and career
                        goals.
                      </p>
                    </div>
                    ...
                    >
                      <div
                        className={`absolute left-0 top-0 bottom-0 w-1 bg-violet-400 rounded-full transition-all duration-300 ${hoveredDevMatchSection === "why" ? "scale-y-100 opacity-100" : "scale-y-0 opacity-0"}`}
                      />
                      <h3 className="text-white font-semibold text-sm">Why DevMatch?</h3>
                      <p className="text-white/80 text-xs leading-relaxed">
                        We use intelligent matching algorithms to find the perfect fit for your next role or project.
                      </p>
                    </div>
                    ...
                    >
                      <div
                        className={`absolute left-0 top-0 bottom-0 w-1 bg-violet-400 rounded-full transition-all duration-300 ${hoveredDevMatchSection === "how" ? "scale-y-100 opacity-100" : "scale-y-0 opacity-0"}`}
                      />
                      <h3 className="text-white font-semibold text-sm">How to Use DevMatch</h3>
                      <p className="text-white/80 text-xs leading-relaxed">
                        Create a profile, set your preferences, and let our system match you with ideal opportunities.
                      </p>
                    </div>
                  </div>
                </div>
            ...
            {isAccountOpen && (
              <div className="absolute top-full mt-0 right-0 w-30 pt-2">
                <div className="bg-black/95 backdrop-blur-md border border-violet-500/20 rounded-xl p-1 space-y-2">
                  ...
                  >
                    <div
                      className={`absolute left-1 top-1/2 -translate-y-1/2 w-1 h-6 bg-violet-400 rounded-full transition-all duration-300 ${hoveredAccountSection === "signup" ? "scale-x-100 opacity-100" : "scale-x-0 opacity-0"}`}
                    />
                    Sign Up
                  </button>
                  <button
                  ...
                  >
                    <div
                      className={`absolute left-1 top-1/2 -translate-y-1/2 w-1 h-6 bg-violet-400 rounded-full transition-all duration-300 ${hoveredAccountSection === "login" ? "scale-x-100 opacity-100" : "scale-x-0 opacity-0"}`}
                    />
                    Log In
                  </button>
                </div>
